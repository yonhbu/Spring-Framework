...
public class ApplicationTests {
    ...
    public void testGetAllEvents() throws Exception {
        ...
        //test by public
        actualRecords = om.readValue(mockMvc.perform(get("/events?public=true"))
                .andDo(print())
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString(), new TypeReference<List<Event>>() {
        });
        Collections.sort(actualRecords, Comparator.comparing(Event::getId));

        List<Event> _expectedRecords = expectedRecords.stream().filter(e -> e.getIsPublic()).collect(Collectors.toList());
        Collections.sort(_expectedRecords, Comparator.comparing(Event::getId));

        for (int i = 0; i < _expectedRecords.size(); i++) {
            Assert.assertTrue(new ReflectionEquals(_expectedRecords.get(i)).matches(actualRecords.get(i)));
        }

        //test by false
        actualRecords = om.readValue(mockMvc.perform(get("/events?public=false"))
                .andDo(print())
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString(), new TypeReference<List<Event>>() {
        });
        Collections.sort(actualRecords, Comparator.comparing(Event::getId));

        _expectedRecords = expectedRecords.stream().filter(e -> !e.getIsPublic()).collect(Collectors.toList());
        Collections.sort(expectedRecords, Comparator.comparing(Event::getId));
    ...
    public void testGetEventByRepos() throws Exception {
        ...
        //public=false/true
        List<Event> actualRecords = om.readValue(mockMvc.perform(get("/repos/1/events?public=true"))
                .andDo(print())
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString(), new TypeReference<List<Event>>() {
        });
        Collections.sort(actualRecords, Comparator.comparing(Event::getId));

        List<Event> expectedRecords = expectedEventsWithRepoId1.stream().filter(e -> e.getIsPublic()).collect(Collectors.toList());
        Collections.sort(expectedRecords, Comparator.comparing(Event::getId));
    ...
    public void testGetEventByUser() throws Exception {
        ...
        //public=false/true
        List<Event> actualRecords = om.readValue(mockMvc.perform(get("/users/1/events?public=true"))
                .andDo(print())
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString(), new TypeReference<List<Event>>() {
        });
        Collections.sort(actualRecords, Comparator.comparing(Event::getId));

        List<Event> expectedRecords = expectedEventsWithUserId1.stream().filter(e -> e.getIsPublic()).collect(Collectors.toList());
        Collections.sort(expectedRecords, Comparator.comparing(Event::getId));
    ...
    public void testNotAllowedMethod() throws Exception {
        ...
        mockMvc.perform(put("/events/" + actualRecord.getId()))
                .andExpect(status().isMethodNotAllowed());

        mockMvc.perform(patch("/events/" + actualRecord.getId()))
                .andExpect(status().isMethodNotAllowed());

        mockMvc.perform(delete("/events/" + actualRecord.getId()))
                .andExpect(status().isMethodNotAllowed());
